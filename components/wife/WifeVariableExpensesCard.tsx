...
import type { WifeVariableExpense } from '@/lib/types'
import { CATEGORIES, ACCOUNTS } from '@/lib/constants'
import { fmt, badgeClass, defaultDateForMonth } from '@/lib/utils'
import { addWifeVariableExpense, deleteWifeVariableExpense } from '@/lib/supabase'
...
interface Props {
  year: number
  month: number
  expenses: WifeVariableExpense[]
  onUpdate: () => void
}

export default function WifeVariableExpensesCard({ year, month, expenses, onUpdate }: Props) {
  ...
  const handleDelete = async (id: string) => {
    await deleteWifeVariableExpense(id)
    onUpdate()
  }
  ...
  return (
    <div className="card mb-4">
      <div className="flex items-center justify-between mb-3">
        <p className="section-title mb-0">Dépenses variables</p>
      ...
      {open && (
        <div className="border-t border-border-light dark:border-border-dark pt-4 mb-3">
          <div className="grid grid-cols-3 gap-2 mb-2">
            <input className="col-span-1" placeholder="Libellé" value={label} onChange={(e) => setLabel(e.target.value)} />
          ...
          return (
            <div key={e.id} className="row-item">
              ...
              <div className="flex items-center gap-2">
                <span className={`badge ${badgeClass(e.account)}`}>{e.account}</span>
